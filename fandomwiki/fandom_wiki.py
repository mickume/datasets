...
from fandom_wiki_util import safe_open_w
...
while nextpage_url != "":
    ...
    try:
        ...
        if content:
            ...
            payload = {'catname': '', 'pages': listofpages, 'curonly': '1',
                       'wpDownload': 1, 'wpEditToken': '+\\', 'title': 'Special:Export'}
            response = requests.post(
                "https://"+fandom_site+".fandom.com/wiki/Special:Export", data=payload)
            data = response.content

            # Create directory 'fandom_site' if it doesn't exist
            with safe_open_w(f"{base_dir}/tmp/{counter}.xml") as f:
                f.write(data)
            counter += 1
        else:
...
files = os.listdir(f"{base_dir}/tmp")

# Create a new file to write to
with open(f"{base_dir}/{fandom_site}.xml", "w") as outfile:
    # Loop through all files in the directory
    for fname in files:
        # Open each file and read it as a MediaWiki XML file
        with open(f"{base_dir}/tmp/{fname}", "r") as infile:
